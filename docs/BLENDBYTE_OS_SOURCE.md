...
Versão desta fonte: 23 de julho de 2026 — catálogo comercial, orçamentos simplificados e apresentação compacta das aprovações
...
Na aprovação por link, Guilherme ou Marketing selecionam o cliente e o mês, confirmam os conteúdos incluídos, agrupam livremente variantes que partilham o mesmo visual e carregam imagens PNG, JPEG ou WebP. A preparação tem uma pré-visualização integral antes da partilha. Ao criar o link, os visuais são enviados diretamente do browser autenticado para o bucket privado do Supabase, sem atravessar a função da Vercel; a ação servidor confirma depois caminho, tipo e tamanho de cada objeto antes de gravar a ronda. Uploads parciais são removidos quando a criação falha. Depois da criação, o modal mantém-se aberto, apresenta o link para copiar ou abrir e sugere um email com assunto e mensagem editáveis para copiar.

A página do cliente apresenta os logótipos BlendByte e do cliente, uma tabela mensal simples com informação prévia dos blocos e, depois, os blocos de conteúdos. Na pré-visualização interna e na página final, cada bloco apresenta diretamente os visuais à esquerda e as descrições dos conteúdos à direita, sem tabs nem exposição do texto do criativo. Cada visual usa a mesma moldura compacta, independentemente de o bloco ter um, dois ou mais conteúdos, e é reduzido proporcionalmente dentro dos limites da moldura para que posts, carrosséis e stories apareçam sempre completos, sem recorte nem distorção. Não é apresentada qualquer legenda sob a imagem. Quando existem vários visuais no mesmo bloco, formam uma grelha compacta em ecrãs largos e uma faixa horizontal navegável em ecrãs pequenos, limitando o scroll e mantendo uma dimensão legível. Um bloco sem imagens mostra apenas as descrições, sem avisos nem espaços vazios sobre essa ausência. Em cada bloco, o cliente aprova ou pede alterações e deixa um comentário. No final, a pessoa confirma o nome e o email, pode acrescentar uma nota geral e envia todas as decisões numa única resposta; os dados do destinatário definidos na preparação surgem pré-preenchidos. O link contém um token aleatório; a base de dados guarda apenas o hash desse token. Por isso, o histórico não revela o link já emitido: admin ou marketing podem gerar um novo, invalidando imediatamente o anterior.
...
O bucket `content-review-assets` é privado, aceita PNG, JPEG e WebP até 8 MB por ficheiro e só pode ser gerido por admin e marketing autenticados. Os uploads de preparação usam caminhos aleatórios sob o cliente e são validados novamente no servidor quanto ao caminho esperado, MIME e tamanho antes de serem associados à ronda. A página pública recebe URLs assinados de curta duração, gerados no servidor depois de validar o token da ronda. As tabelas da aprovação não dão acesso a `anon`; a leitura e escrita do cliente passam por ações servidor com service role e validação explícita do token.
