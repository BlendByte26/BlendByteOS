...
export const metadata: Metadata = {
  title: "Aprovação de conteúdos | BlendByte",
  description: "Aprovação privada de um planeamento de conteúdos.",
  robots: { index: false, follow: false, nocache: true },
};

export default async function PublicContentApprovalPage({ params }: { params: Promise<{ token: string }> }) {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#efefeb] p-5">
        <section className="w-full max-w-lg rounded-[28px] border border-[var(--bb-border)] bg-white/85 p-7 text-center shadow-[0_24px_80px_rgba(0,0,0,0.12)]">
          ...
          <h1 className="mt-6 text-2xl font-extrabold text-[var(--bb-charcoal)]">Esta aprovação não está disponível</h1>
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(83,183,223,0.16),transparent_34%),#efefeb] px-3 py-5 md:px-6 md:py-8">
      <div className="mx-auto max-w-6xl">
        ...
        <footer className="px-3 py-8 text-center text-xs font-bold text-[var(--bb-muted)]">Página privada de aprovação · BlendByte</footer>
      </div>
    </main>
  );
}
