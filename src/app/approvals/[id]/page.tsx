...
export default async function ApprovalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <PageHeader title={`Aprovação · ${review.client_name}`} description={`${contentReviewStatusLabels[review.status]} · ${summary.approved} aprovados · ${summary.changes} com alterações`} action={<SecondaryLink href="/approvals">Ver aprovações</SecondaryLink>} />
      ...
      {review.status === "changes_requested" ? (
        <section className="mb-6 rounded-[24px] border border-[rgba(232,76,49,0.25)] bg-[var(--bb-red-soft)] p-5">
          ...
          <p className="mt-1 text-sm font-semibold text-[#8f2415]/80">Inicie uma revisão por bloco. A ação reabre os conteúdos originais e cria uma tarefa operacional — nunca uma tarefa de Design.</p>
